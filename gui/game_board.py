"""
Game board widget for the Organ Attack GUI.
Displays the main game area with player panels and game controls.
"""
...
from game.models import GameState, CardType
...
class GameBoard(ttk.Frame):
    ...
    def _create_layout(self):
        ...
        self.current_player_frame.grid(row=1, column=0, sticky="nsew", padx=5, pady=5)
    ...
    def _create_player_panels(self):
        ...
        self.current_player_panel.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def _update_button_states(self):
        ...
        elif game_state == GameState.DEFEND:
            if self.engine.pending_defense:
                self.action_buttons['skip_defense'].config(state='normal')
    ...
    def _skip_defense(self):
        """Handle skip defense button."""
        result = self.engine.skip_defense()
        if result['success']:
            self.main_window._update_status("Attack not defended")
        self.main_window._update_game_display()
    ...
    def _show_deck_info(self):
        ...
        info_window.transient(self.main_window.root)
        info_window.grab_set()
        
        ttk.Label(info_window, text=info_text, font=('Arial', 12)).pack(pady=20)
