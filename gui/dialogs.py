...
class NewGameDialog:
    ...
    def _ok_clicked(self):
        ...
        if any(len(name) > 20 for name in names):
            messagebox.showerror("Error", "Player names must be 20 characters or less!")
            return
...
class LoadGameDialog:
    ...
    def _load_saves(self):
        ...
        if not saves:
            # No saves found
            self.tree.insert('', tk.END, values=('No saved games found', '', '', ''))
            return
        ...
        for save in saves:
            ...
            self.tree.insert('', tk.END, values=(name, players, turn, saved_at))
    
    def _load_clicked(self):
        """Handle Load button click."""
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("Warning", "Please select a saved game to load!")
            return
    ...
    def _delete_clicked(self):
        ...
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("Warning", "Please select a saved game to delete!")
            return
