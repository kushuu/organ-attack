...
class CardWidget(ttk.Frame):
    ...
    def __init__(self, parent, card: Card, engine: GameEngine, main_window, 
                 show_details: bool = True, clickable: bool = False):
    ...
    def _create_layout(self):
        ...
        # Get card colors
        color_scheme = self.colors.get(self.card.type, {'bg': '#95a5a6', 'fg': 'white'})
    ...
    def _on_click(self, event):
        ...
        if game_state.name == 'DEFEND' and self.card.type != CardType.DEFENSE:
            self.main_window._update_status("Can only play defense cards during defend phase")
            return
        
        if game_state.name not in ['PLAY', 'DEFEND']:
            self.main_window._update_status(f"Cannot play cards during {game_state.name} phase")
            return
    ...
    def _get_targets(self):
        ...
        selector = TargetSelector(self.main_window.root, self.card, self.engine)
