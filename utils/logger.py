...
class GameEventLogger:
    ...
    def log_attack_result(self, attacker: str, defender: str, card: str,
        ...
        if success:
            self.logger.info(
                f"ATTACK SUCCESS: {attacker}'s {card} destroyed {defender}'s {target_organ}")
        else:
            block_info = f" (blocked by {blocked_by})" if blocked_by else ""
            self.logger.info(
                f"ATTACK BLOCKED: {attacker}'s {card} vs {defender}'s {target_organ}{block_info}")

    def log_player_elimination(self, player_name: str, turn_number: int):
        """Log a player being eliminated."""
        self.logger.warning(
            f"PLAYER ELIMINATED: {player_name} on turn {turn_number}")
